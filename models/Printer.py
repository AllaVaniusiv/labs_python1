...
class Printer(ABC):
    ...
    def __str__(self):
        ...
        return f"""Model: {self.model}, Pr Type: {self.pr_type}, Color: {self.is_color}, Duplex: {self.is_duplex},Paper Tray Capacity: {self.paper_tray_capacity}, Paper Count: {self.paper_count}"""

    def __repr__(self):
        """Returns a string that is a valid Python expression for reproducing the object."""
        return f"Printer(model='{self.model}', pr_type='{self.pr_type}', is_color={self.is_color}, is_duplex={self.is_duplex}, paper_tray_capacity={self.paper_tray_capacity}, paper_count={self.paper_count})"

   
    @staticmethod
    def get_instance():
        """
        Get the default instance of the Printer class.

        Returns:
            Printer: The default Printer instance.
        """
